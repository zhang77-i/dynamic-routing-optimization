...
- PyVRP：多起点静态距离基线，每名骑手建模为一个车辆类型，以零成本返仓弧匹配开放路线口径；
...
| 方法 | 平均投影距离 | 合成 SLA 迟到率 |
| --- | ---: | ---: |
| PyVRP 距离基线 | 44.98 km | 55.39% |
| 自建离线 ALNS | 50.75 km | 50.34% |
| Regret-2 | 72.42 km | 60.59% |

PyVRP 的距离最短，但它没有直接优化本项目的合成 SLA 与负载均衡项，所以不能据此宣称整体方案更优；这个对照用于检查模型目标与业务指标是否一致。
...
- PyVRP 基线使用整数化投影距离，只作为外部静态求解器参照；它不直接优化本项目的合成 SLA 迟到和负载均衡项。
